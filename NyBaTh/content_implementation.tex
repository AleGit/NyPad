...
with an exercise button on the top of the page.
...
The user can use a formula from this list to add an syntax view to the detail view - the canvas.
Or the user can add a new syntax tree to the canvas.
...
\subsection{On delivery}

Xml configuration data and html documents are stored in UTF-8 encoded files. 
The content is embedded in \Nyaya's app bundle,
which can be downloaded and installed from the App Store. 

\subsection{Persisted formulas}

Formulas from the playground and BoolTool are persisted in the simple property list file \verb+BoolToolData+ 
