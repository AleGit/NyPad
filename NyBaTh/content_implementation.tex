...
Ny$\bar{a}$ya's main navigation – switching between Welcome, Tutorials, Playground, Glossary and NyBoolTool – 
is organized by a sub-class of\verb+ UITabBarController+ (an heir of \verb+UIViewController+). 
At start the tab bar controller will be instantiated and invoked by Ny$\bar{a}$ya's application delegate. 
...
The glossary master view controller presents a table view with the list of technical terms,
which will be filled by the anchor embedded the glossary document.
Tapping an term will scroll the web view to the term, 
