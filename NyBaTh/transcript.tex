...
Now we can calculate the valuation, and we realize that the condition "p" is not true,
...
BoolTool is a powerful means for manipulating and evaluating formulas in propositional logic.  
...
and the coherence of different representations of a Boolean function.
...
by combining small bits of learning content with literally countless exercises.

Nyaya offers short tutorials for general concepts and definitions as well as 
exercises to consolidate the learned content and definitions. 
...
but I used some principles of agile development and scrum instead. 
...
I also created several tests before its implementation, which I used to prove the correct running of the application at any time of the process. 
This principle is called test driven development. While implementing, I refactored many lines of code. 
...
\frame{
\frametitle{Model}
The scanner of Nyaya uses a Regex to transform the input string into a stream of tokens. 
Special symbols and words are transformed into operator tokens, all other words are recognized as identifiers.
Since the cocoa framework works with Unicode-strings, you can use Chinese words as identifiers.
For the parser, I adjusted an EBNF grammar for propositional logic to bring it into accordance with
precedences and associativity used in BoolTool. 
This grammar was implemented straight forward into a recursive descendent parser,
following the principles outlined in "Compiler Construction" by Kenneth C. Louden.
...
In Nyaya syntax trees are designed as immutable reduced acyclic object graphs.
Every syntax operation will create a new acyclic object graph by re-using sub-formulas multiple times.
...
If there is a typo in a tutorial, a new version of Nyaya has to be submitted to the app store.
It usually takes one week from submitting to publishing an app. 
Since all content files are html files, it would be possible for Nyaya to download directly from a web server, 
but this feature has not been added yet.
...
You can paste formulas into the input field of bool tool, but you cannot export normal forms.
...
A German version will be released this year.
}
...
Actually the parser would support it, but Nyaya's syntax tree and BoolTool do not.
...
You will get a promo code to download it for free, 
when you write an E-Mail to nyaya@maringele.at 
with a reply-address of the University of Innsbruck. 
You can also download the source code from GitHub and run it on a Mac with Xcode. 
To install Nyaya on your iPad, you will need a developer certificate from Apple. 
Or you have jailbreaked your device. 
...
The content is completely based on "Logic in Computer Science" by Huth and Ryan and the Logic Lecture of professor Middeldorp.
The architecture of Nyaya was guided by "design patterns: elements of reusable object-oriented software" by the Gang of Four. 
Nyaya's Parser was built following principles taken from "compiler construction" by Louden. 
The platform-specific tasks were mastered with the help of "cocoa design patterns" by Buck anydYacktman
